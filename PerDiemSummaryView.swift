...
struct PerDiemView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingRateSettings) {
            PerDiemRateSettingsView(store: store, airlineSettings: airlineSettings)
        }
        .sheet(isPresented: $showingPortalInfo) {
            if let period = selectedPeriodForPortal {
                PortalEntryInfoView(portion: period, rate: store.perDiemRate)
            }
        }
    }
...
struct PortalEntryInfoView: View {
