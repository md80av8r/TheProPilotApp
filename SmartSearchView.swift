...
struct SmartSearchView: View {
    ...
    private var emptyStateView: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                SmartSearchQuickButton(title: "GPS tracking", icon: "location.fill") {
                    searchText = "GPS tracking"
                }

                SmartSearchQuickButton(title: "Auto time logging", icon: "clock.arrow.2.circlepath") {
                    searchText = "Auto time"
                }

                SmartSearchQuickButton(title: "Airport database", icon: "building.2.crop.circle") {
                    searchText = "Airport"
                }

                SmartSearchQuickButton(title: "NOC import", icon: "calendar.badge.clock") {
                    searchText = "NOC"
                }
            }
            .padding(.top, 20)
        ...
        Group {
            if !filteredAppItems.isEmpty {
                Section {
                    ...
                    SmartSearchSectionHeader(title: "App Features & Settings", icon: "square.grid.2x2", resultCount: filteredAppItems.count)
                }
            }
        }
    }
                ...
                Section {
                    VStack(spacing: 0) {
                        ForEach(filteredHelpArticles.prefix(10)) { article in
                            SmartSearchHelpRow(article: article)
                    ...
                    SmartSearchSectionHeader(title: "Help & Support", icon: "questionmark.circle", resultCount: filteredHelpArticles.count)
                }
            }
        }
    }
        ...
        Group {
            if !filteredFlights.isEmpty {
                Section {
                    ...
                    SmartSearchSectionHeader(title: "Flight Logs", icon: "airplane", resultCount: filteredFlights.count)
                }
            }
        }
    }
    ...
    private var filteredFlights: [Trip] {
        guard !searchText.isEmpty else { return [] }
        return store.trips.filter { trip in
            // Search trip number
            if !trip.tripNumber.isEmpty && trip.tripNumber.localizedCaseInsensitiveContains(searchText) {
                return true
            }

            // Search aircraft
            if !trip.aircraft.isEmpty && trip.aircraft.localizedCaseInsensitiveContains(searchText) {
                return true
            }

            // Search legs for airports
            for leg in trip.legs {
                // Check departure airport
                if leg.departure.localizedCaseInsensitiveContains(searchText) {
                    return true
                }
                // Check arrival airport
                if leg.arrival.localizedCaseInsensitiveContains(searchText) {
                    return true
                }
            }
...
// MARK: - Section Header (renamed to avoid conflicts)
struct SmartSearchSectionHeader: View {
    let title: String
    let icon: String
    let resultCount: Int
...
// MARK: - App Feature Row (renamed to avoid conflicts)
struct SmartSearchAppRow: View {
...
// MARK: - Help Article Row (renamed to avoid conflicts)
struct SmartSearchHelpRow: View {
...
// MARK: - Logbook Search Row (renamed to avoid conflicts)
struct SmartSearchTripRow: View {
    let trip: Trip
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let firstLeg = trip.legs.first, let lastLeg = trip.legs.last {
                    let depAirport = firstLeg.departure.isEmpty ? "???" : firstLeg.departure
                    let arrAirport = lastLeg.arrival.isEmpty ? "???" : lastLeg.arrival
                    Text("\(depAirport) → \(arrAirport)")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            }
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text(String(format: "%.1f", Double(trip.totalFlightMinutes) / 60.0))
                    .font(.caption)
                    .fontWeight(.medium)
...
// MARK: - Quick Search Button (renamed to avoid conflicts)
struct SmartSearchQuickButton: View {
    let title: String
    let icon: String
    let action: () -> Void
...
struct SearchableHelpArticle: Identifiable {
    ...
    static let allArticles: [SearchableHelpArticle] = [
        SearchableHelpArticle(
            ...
            content: "Record your flight paths automatically with GPS Track Recording."
        ),
        SearchableHelpArticle(
            ...
            content: "GPS-based automatic time capture with takeoff and landing detection."
        ),
        SearchableHelpArticle(
            ...
            content: "Import your roster automatically from NOC schedule emails."
        ),
        SearchableHelpArticle(
            title: "Smart Search",
            category: "Features",
            preview: "Unified search for app features, help articles, and flight logs",
            keywords: ["search", "find", "lookup", "smart", "unified"],
            content: "Find anything instantly with Smart Search - search app features, help articles, and your flight logs all in one place."
        ),
    ]
}
